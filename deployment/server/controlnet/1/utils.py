...
def device_view(t):
    # warning: this does not keep a reference to the tensor object,
    # if it goes out of scope and gets garbage collected, the data ptr will be invalid
    assert t.is_contiguous(), 'Tensor is not contiguous'  # avoid future headaches
...
class Engine:
    def __init__(self, engine_path):
        ...
        self.engine = None
        self.context = None
        # stream 0 is the same one used by default PyT
        # we use it so we don't have to worry about inter-streams dependencies
        self.stream = 0
    ...
    def infer(self, feed_dict):
        ...
        noerror = self.context.execute_async_v2(bindings=bindings, stream_handle=stream)
        if not noerror:
