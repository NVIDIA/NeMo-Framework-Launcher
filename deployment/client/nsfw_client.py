...
def main():
    ...
    triton_output_grpc = triton_grpc.InferRequestedOutput("nsfw_probs")
    request_grpc = grpc_client.infer(
        "nsfw_trt", model_version="1", inputs=triton_input_grpc, outputs=[triton_output_grpc]
    )
    outt = request_grpc.as_numpy("nsfw_probs")
