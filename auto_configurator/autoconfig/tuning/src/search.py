...
from src.utils import (modify_cfg, generate_finetuning_overrides_str,
                       submit_slurm_job, load_slurm_params_from_sh)
...
def run_search(cfg: omegaconf.dictconfig.DictConfig) -> None:
    ...
    cfg.base_results_dir = os.path.join(cfg.base_results_dir, cfg.search_config.model_name, start_time)
...
def run_finetuning_job(
    ...
    nemo_megatron_ci = ("NEMO_LAUNCHER_CI=1" if bool(os.getenv("NEMO_LAUNCHER_CI")) else "")
...
def launch_result_analysis(cfg: omegaconf.dictconfig.DictConfig, job_ids: List[int], start_time: str) -> None:
    """
    ...
    slurm_script = "\n".join([
        "#!/bin/bash\n",
        "\n".join(f"#SBATCH {key}={str(value)}" for key, value in slurm_args.items()),
