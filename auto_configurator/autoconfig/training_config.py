...
def generate_grid_search_configs(
    ...
    multiplier = (
        1 if model_name in ["gpt3", "bert", "llama", "baichuan2", "chatglm", "qwen2"] else 2
    )
...
def _set_activations_checkpoint_params(
    ...
    if (
        model_name in ["gpt3", "bert", "llama", "baichuan2", "chatglm", "qwen2"] and pp > 2
    ...
    if act_method == "block":
        ...
        if pp > 1 and model_name in ["gpt3", "bert", "llama", "baichuan2", "chatglm", "qwen2"]:
            # Num micro batches with partial act ckpt
...
def _calculate_tp_pp_mbs_grid(
    ...
    init_pp = [] if model_name in ["gpt3", "llama", "baichuan2", "chatglm", "qwen2"] else [1]
    valid_pp = init_pp + [
